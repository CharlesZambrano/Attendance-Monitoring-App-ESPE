...
const ClassCard = ({ schedule, onCardClick }) => {
  const { CLASS_SCHEDULE_ID, SUBJECT, NRC, TYPE, START_TIME, END_TIME } =
    schedule;
    ...
    const fetchAttendanceInfo = async () => {
      try {
        const response = await fetch(
          `${API_ENDPOINTS.GET_CLASS_SCHEDULE_ATTENDANCE}/${CLASS_SCHEDULE_ID}`
        );
        ...
        if (response.ok && data.length > 0) {
          ...
          console.warn(
            `No attendance records found for CLASS_SCHEDULE_ID ${CLASS_SCHEDULE_ID}`
          );
        }
      } catch (error) {
  ...
  return (
    <div className="class-card" onClick={onCardClick}>
      ...
      <div className="card-content">
        <h3>{SUBJECT}</h3>
        <p>
          <strong>NRC:</strong> {parseInt(NRC, 10)}
        </p>{" "}
        {/* Convert NRC to an integer */}
        <p>
        ...
        <p>
          ...
          {attendanceInfo?.ENTRY_TIME && (
            <span className="aligned-right">
              <strong>Hora de registro Inicio:</strong>{" "}
              {formatTimeOnly(attendanceInfo.ENTRY_TIME)}{" "}
              {attendanceInfo.LATE_ENTRY === "SI" && (
                <span className="late-text">
                  (Tarde{" "}
                  <img src={speechIcon} alt="Tarde" className="late-icon" />)
                </span>
              )}
            </span>
          )}
        ...
        <p>
          ...
          {attendanceInfo?.EXIT_TIME && (
            <span className="aligned-right">
              <strong>Hora de registro Fin:</strong>{" "}
              {formatTimeOnly(attendanceInfo.EXIT_TIME)}{" "}
              {attendanceInfo.LATE_EXIT === "SI" && (
                <span className="late-text">
                  (Tarde{" "}
                  <img src={speechIcon} alt="Tarde" className="late-icon" />)
                </span>
